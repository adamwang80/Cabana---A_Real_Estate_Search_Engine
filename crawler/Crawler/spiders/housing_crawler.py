...
class QuotesSpider(CrawlSpider):
    ...
    rules = (
        # Rule(LinkExtractor(allow = "www.zillow.com", restrict_css=".pfs__sc-1elvxkv-1.pfs__sc-1elvxkv-2.pfs__sc-1wickoz-0.fzMoEi > li > a")),
        Rule(LinkExtractor(allow = "https://www.zillow.com/homes/for_rent/", restrict_css=".search-list-relaxed-results > ul > li > article > div > div > a"), callback="parse_zillow"),
    ...
    def parse_zillow(self, response):
        print("in parse zillow")
        name = response.css('.BuildingInfo__BuildingInfoContainer-d8oth5-2.igICEi > h1::text').get()
        print("name: ", name)
